...
# /usr/bin/time snakemake -p -s simulate_genomes.smk --configfile ../simulation-configs/mm39-iterative.yaml --profile profiles/slurm_profile/ --dryrun --rulegraph | dot -Tpng > dags/simulate-genomes-dag.png
...
rule merge_divergent_golden_vcfs:
    ...
    output:
        merged_vcf = os.path.join(outdir, "simulated-reads", "{cov}X", "{div}", "divergent", ref_str + "_golden.vcf.gz"),
        merged_vcf_index = os.path.join(outdir, "simulated-reads", "{cov}X", "{div}", "divergent", ref_str + "_golden.vcf.gz.tbi")
    log:
