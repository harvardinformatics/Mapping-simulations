...
rule all:
    input:
        expand(os.path.join(outdir, "summary-files", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}-compare-vcf-summary.csv"), cov=covs, div=divs, region=regions),
        expand(os.path.join(outdir, "summary-files", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}-compare-vcf-snps.csv"), cov=covs, div=divs, region=regions),
        # Expected output from compare_vcfs

        expand(os.path.join(outdir, "summary-files", "{cov}X", "{div}", ref_str + "-{cov}X-{div}-compare-bam-summary.csv"), cov=covs, div=divs),
...
rule map_simulated_reads:
    ...
    output:
        os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam")
    params:
        read_group = "@RG\\tID:" + ref_str + "-{cov}X-{div}\\tLB:NEAT\\tPL:ILLUMINA" + "\\tSM:" + ref_str + "-{cov}X-{div}"
    log:
        os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", "logs", ref_str + "-{cov}X-{div}.log")
    resources:
        mem= "24g",
        cpus = 12,
        time = "16:00:00"
...
rule index_bams:
    input:
        os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam")
    output:
        os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam.bai")
    log:
        os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", "logs", ref_str + "-{cov}X-{div}-index.log")
    resources:
        time = "4:00:00",
        mem = "4g"
    shell:
...
rule gatk_haplotypecaller: 
    input:
        bam = os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam"),
        bam_index = os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam.bai"),
        ref = REF
    output:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}.gvcf.gz")
    params:
        region = "{region}"
    log:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "logs", ref_str + "-haplotypecaller-{region}-{cov}X-{div}.log"),
    resources:
        cpus = 4,
        mem="4g",
...
rule gatk_genotypegvcfs: 
    input:
        gvcf = os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}.gvcf.gz"),
        ref = REF
    output:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}.vcf.gz")
    params:
        region = "{region}"
    log:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "logs", ref_str + "-genotypegvcfs-{region}-{cov}X-{div}.log"),
    resources:
        mem="4g",
        time = get_runtime_gatk
    shell:
...
rule index_vcfs_regions:
    input:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}.vcf.gz")
    output:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "regions", ref_str + "-{region}-{cov}X-{div}.vcf.gz.tbi")
    log:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "logs", ref_str + "-{region}-{cov}X-{div}-tabix.log")
    resources:
        mem = "2g",
        time = "2:00:00"
    shell:
...
rule merge_vcfs:
    input:
        vcf = expand(os.path.join(outdir, "called-variants", "gatk", "{{cov}}X", "{{div}}", "regions", ref_str + "-{region}-{{cov}}X-{{div}}.vcf.gz"), region=regions),
        index = expand(os.path.join(outdir, "called-variants", "gatk", "{{cov}}X", "{{div}}", "regions", ref_str + "-{region}-{{cov}}X-{{div}}.vcf.gz.tbi"), region=regions)
    output:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", ref_str + ".vcf.gz"),
    log:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "logs", ref_str + "-vcf-merge.log"),
    resources:
        cpus = 8,
        time = "2:00:00",
        mem = "4g"
...
rule index_vcfs_merged:
    input:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", ref_str + ".vcf.gz")
    output:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", ref_str + ".vcf.gz.tbi")
    log:
        os.path.join(outdir, "called-variants", "gatk", "{cov}X", "{div}", "logs", ref_str + "-tabix.log")
    resources:
        mem = "2g",
        time = "2:00:00"
    shell:
...
rule compare_bams:
    input:
        ...
        query = os.path.join(outdir, "mapped-reads", "{cov}X", "{div}", ref_str + "-{cov}X-{div}.bam"),
        index = REF_INDEX
    params:
       div = "{div}",
       cov = "{cov}"
