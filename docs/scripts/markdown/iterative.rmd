...
# Sanity check: simulated divergence vs. % of sites simulated with SNPs for iteration 1

Just to make sure the simulations are working

```{r sim-check, out.width="50%", fig.align = "center", warning=FALSE}

vcf_comp_iter1 = vcf_comp %>% filter(iteration == 1)
# Get only SNP sites from iteration 1

sim_vars_iter1 = vcf_comp_iter1 %>% filter(ref != golden.div)
# Get only sites where the reference allele does not match the golden allele (a simulated substitution)

sim_vars_div_iter1 = sim_vars_iter1 %>% group_by(divergence) %>% summarize(n=n()) %>% mutate(perc.var.sites=n/total_len)
# Group the SNPs and count them
...
het_sites = vcf_comp_iter1 %>% filter(iter.gt == "het") %>% group_by(divergence) %>% summarize(n=n()) %>% mutate(perc.het.sites=n/total_len)
...
tp = vcf_comp_iter1 %>% filter(golden.div != ref & iter.gt == "hom.alt" & iter.allele.1 == golden.div) %>% group_by(divergence, iteration) %>% summarize(n=n(), class="tp")
fp = vcf_comp_iter1 %>% filter(golden.div == ref & iter.gt == "hom.alt")  %>% group_by(divergence, iteration) %>% summarize(n=n(), class="fp")
fn = vcf_comp_iter1 %>% filter(golden.div != ref & iter.gt == "hom.prev" & iter.allele.1 == ref)  %>% group_by(divergence, iteration) %>% summarize(n=n(), class="fn")
...
snp_rates_p = ggplot(subset(snp_classes, class != "tp" & divergence > 0), aes(x=divergence, y=prop, color=class)) +
  ...
  scale_color_manual(guide=guide_legend(reverse=T), labels=c("False negative", "False positive"), values=cols) +
  bartheme() +
  theme(legend.position="bottom")
print(snp_rates_p)
...
# Iteration vs. total called variants
...
minimap_vars = vcf_comp %>% filter(minimap.gt == "hom.alt")
minmap_vars_iter = minimap_vars %>% group_by(divergence, iteration) %>% summarize(n=n()) %>% mutate(perc.called.sites=n/total_len)
minmap_vars_iter$iteration = as.character(minmap_vars_iter$iteration)

cols = corecol(numcol=3, pal="wilke")

div_called_p = ggplot(minmap_vars_iter, aes(x=divergence, y=perc.called.sites, color=iteration)) +
  geom_point(size=4, alpha=0.5) +
  geom_abline(slope=1, intercept=0, size=2, linetype="dashed", color="#999999") +
...
tp = vcf_comp %>% filter(golden.div != ref & minimap.gt == "hom.alt" & minimap.allele.1 == golden.div) %>% group_by(divergence, iteration) %>% summarize(n=n(), class="tp")
fp = vcf_comp %>% filter(golden.div == ref & minimap.gt == "hom.alt")  %>% group_by(divergence, iteration) %>% summarize(n=n(), class="fp")
fn = vcf_comp %>% filter(golden.div != ref & minimap.gt == "hom.ref" & minimap.allele.1 == ref)  %>% group_by(divergence, iteration) %>% summarize(n=n(), class="fn")
...
read_rates_p = ggplot(long_bam_data, aes(x=iteration, y=value, color=name)) +
  geom_line(size=1.2) +
  geom_point(size=3) +
  #geom_text(size=2.3, position=position_stack(vjust=0.5), color="#f2f2f2") +
  ...
  #scale_x_continuous(breaks=unique(long_data$iteration)) +
  scale_color_manual(guide=guide_legend(reverse=T), labels=c("Exact map", "Close map", "Mismapped", "Different chrome", "Unmapped"), values=cols) +
  facet_wrap(~divergence) +
...
#   filter(iteration != "golden") %>%
#   group_by(divergence, iteration) %>% 
#   summarize(mean.snps=mean(snps, na.rm=T), mean.div=mean(snps/length, na.rm=T), total.snps=sum(snps)) %>% 
#   mutate(length=total_len) %>% 
#   mutate(est.div=total.snps/length)
# 
# #window_summary$divergence = as.factor(window_summary$divergence)
# p2 = ggplot(window_summary, aes(x=divergence, y=est.div, color=iteration)) +
#   geom_point(size=3) +
#   bartheme()
# 
# print(p2)
