...
vcf_comp_file = here("data", "no-indels", "mm39-30X-0.005h-3i-vcf-comparison.tsv.gz")
...
mapping_file = here("data", "no-indels", "mm39-30X-0.005h-3i-bam-summary.csv")
bam_stats = read_csv(mapping_file)
...
minimap_vars_iter = vcf_comp %>% filter(iter.gt == "het") %>% 
  group_by(divergence, iteration) %>% 
  summarize(n=n()) %>% 
  mutate(perc.called.sites=n/total_len)
...
p =  windows %>% ggplot(aes(x=start, y=rollmean(est.div, 75, na.pad = TRUE, align = "right"), color=iteration)) +
  geom_line() +
  facet_wrap(~divergence) +
